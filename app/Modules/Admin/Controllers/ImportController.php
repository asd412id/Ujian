  ...
  public function sekolah($data)
  {
    if (is_array($data)) {
      ...
      foreach ($data as $key => $row) {
        ...
        } elseif (trim(strtolower($row[0])) == 'kop kartu') {
          $insert['kop_kartu'] = trim($row[2]);
        } elseif (trim(strtolower($row[0])) == 'tipe kop') {
          $insert['kop_type'] = trim(strtolower($row[2]));
        }
      }
      ...
      $import->kop_kartu = $insert['kop_kartu'];
      $import->kop_type = $insert['kop_type'] != 'tidak ada' ? $insert['kop_type'] : null;
